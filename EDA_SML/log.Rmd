...
```{r}
patient.data <- read.table("Data/csv_Files/parkinsons_data.csv", header = T, sep = ",")
...
definitions <- read.csv("Data/csv_Files/definitions.csv", header = FALSE, sep = ",")
...
```{r}
pander::pander(read.csv("Data/csv_Files/differences_of_classLabels.csv", sep = ",", header = T))
```

From the Welch t-testing of each attribute it seems all p-values are below alpha = 0.5. This means that the data of the class labels differ significantly for each attribute. 
Attribute MDVP.APQ has the largest difference of 3.1e-18 and PPE the second largest of 6.1e-17.
...
Though what does negative correlation really indicate? An article from Indeed states “one variable increases in value while the other decreases“. This statement is what attributes NHR and HNR showed earlier and clarifies that the attributes MDVP.Jitter.Abs and MDVP.Flo.Hz have values that stand face to face. Since the healthy and sick values are both negative with a large difference it really tells how divided the levels are with this attribute combination. Therefore, it's very exhilarating to observe these attributes in later modeling research. 
...
```{r}
experimenter.two.files <- read.csv("Data/Experimenter/default_two_files.csv", header = T, sep = ",")
...
ggplot(best.features.counts, aes(x = features, fill = features)) + geom_bar() + xlab("Feature names") + 
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + geom_text(stat='count', aes(label=after_stat(count)), vjust=1)
```

In the bar plot it becomes clear that five attributes: MDVP.APQ, MDVP.Flo.Hz, MDVP.Fo.Hz, MDVP.Jitter.Abs and spread1 occurred an equal amount of times in the top six.
Attribute PPE occurred in the top six of all evaluations in select features, which is seven evaluations that have been performed.

**Retrospective**

The observation from Cfs actually summarizes the general best placement for all attributes per evaluation method. After comparing all methods PPE seems the best attribute to split on and spread1 as second attribute. Attributes MDVP.Flo.Hz and MDVP.Jitter.Abs are also mainly quite good.
...
```{r}
randomForest.data <- data.frame(read.csv("Data/Final_models/RF_bagging.arff", skip = 18))
...
```{r}
oneR.data <- data.frame(read.csv("Data/Final_models/OneR_stacking.arff", skip = 18))
...
From selecting features and testing WrapperSubsetEval on Random forest the following 6 attributes ranked best :
MDVP.Fo.Hz., MDVP.Flo.Hz.,  MDVP.Jitter.Abs., Shimmer.DDA, NHR, PPE.
...
**Retrospective**
A final model has been found with a 93.3333 percent accuracy; a by bagging optimized random forest model that chooses seven random attributes.
