...
Dataset                |   (1) rules.Zero | (2) rules.One |(3) bayes |(4) trees (J48) |(5) trees (RandomForest) |(6) lazy.
...
What would happen if for J48 the unpruning is set to true with a minNumObj to three and a confidenceFactor of 0.20? Also for OneR the bucketsize is set to 20. What happens if the batchSize is increased to 120, i.e. the amount in instances to process is increased. The amount of KNN is increased to twenty to decrease the overfitting. RandomForest remains the same, because regardless it searches for the best/better outcome. Also by pressing Swap and then e.g. IBk (6) the models are compared to that model. The differences of those models with that model is shown with the (v) and less significant than that is the star (*) symbol.


Dataset                   (1) log_pati | (2) patie
--------------------------------------------------
rules.ZeroR '' 4805554146(100)   75.39 |   75.39  
rules.OneR '-B 20' -34594(100)   85.00 |   85.10  
rules.OneR '-B 6' -345942(100)   87.81 |   87.70  
trees.J48 '-C 0.25 -M 2' (100)   84.63 |   84.79  
trees.J48 '-U -M 3' -2177(100)   84.93 |   84.83  
bayes.NaiveBayes '' 59952(100)   72.75 |   70.14  
bayes.NaiveBayes '-batch-(100)   72.75 |   70.14  
trees.RandomForest '-P 10(100)   91.70 |   91.41  
lazy.IBk '-K 1 -W 0 -A (100)     93.03 |   95.91 v
lazy.IBk '-K 20 -W 0 -A (100)    84.78 |   82.52  
--------------------------------------------------
                               (v/ /*) |   (1/9/0)
...
Let's now look at how OneR performs with the same settings.
...
The third best model is the edited J48 model.


Selected attributes: 1,5,6,22 : 4
                     MDVP.Fo.Hz.
                     MDVP.Jitter.Abs.
                     MDVP.RAP
                     PPE
                     
Again MDVP.Fo.Hz has been predicted as the best attribute. A new attribute in the top three is MDVP.RAP.
...
This evaluation evaluates groups of attributes together
