...
barplot(c(amount.sick.recordings, amount.healthy.recordings), 
        legend.text = c("Sick", "Healthy"), col = c("red", "green"),
...
```{r}
patient.data.numeric <- patient.data[,!names(patient.data) %in% c("status", "name")]
patient.data.numeric
pairs(patient.data)
```
