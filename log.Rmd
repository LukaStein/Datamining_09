...
Flo <- patient.data$MDVP.Flo.Hz.[patient.data$status == "sick"]
...
Flo.healthy <- patient.data$MDVP.Flo.Hz.[patient.data$status == "healthy"]
...
With a ANOVA test these differences can be determined.

```{r}
aov.MDVP.Hz. <- summary(aov(MDVP.Fo.Hz. + MDVP.Fhi.Hz. + MDVP.Flo.Hz. ~ status, data = patient.data))
aov.MDVP.Hz[[1]]$`Pr(>F)`
```
With a p-value of 3.12e-08 < 0.05 there's a significant difference between the attributes and the statuses.

**Retrospective**
There is a significant difference in the attributes MDVP.Fo.Hz, MDVP.Fhi.Hz, MDVP.Flo.Hz and the levels healthy and sick. These attributes might be interesting to use in differentiating people of PD with healthy people.
...
ggplot(difference.in.nums.sick.log10,aes(x=labs, y=values)) + 
  geom_boxplot(fill = "brown1") + ggtitle("Log-fold 10 of MDVP.Hz attributes") + 
  labs(caption = "Normalized boxplot with log10")  + 
  xlab("Sick MDVP attributes") + ylab("Measured values (Hz)")

ggplot(difference.in.nums.healthy.log10,aes(x=labs, y=values)) +
  geom_boxplot(fill = "cyan2") + ggtitle("Log-fold 10 of MDVP.Hz attributes") + labs(caption = "Normalized boxplot with log10") + 
  xlab("Healthy MDVP attributes") + ylab("Measured values (Hz)")
```
...
To see if after normalizing the data really has differences between the data and its levels an ANOVA test is done on the log10 of the attributes.

```{r}
aov.MDVP.Hz.log <- summary(aov(log(MDVP.Fo.Hz.) + log(MDVP.Fhi.Hz.) + log(MDVP.Flo.Hz.) ~ status, data = patient.data))
aov.MDVP.Hz.log[[1]]$`Pr(>F)`
```
After normalizing the data, compared to the previous p-value of 3.12e-08, an even stronger significant difference is portrayed with a p-value of 5.48e-08 < 0.05. 
...
These new normalised data will be added to the data set, for processing later on.

\pagebreak


Writing log-transformed data to new file for model training later.


```{r}
logTransformed.patient.data <- data.frame(log.MDVP.Fhi.Hz. = log(patient.data$MDVP.Fhi.Hz.),
                                    log.MDVP.Flo.Hz. = log(patient.data$MDVP.Flo.Hz.),
                                    log.MDVP.Jitter = log(patient.data$MDVP.Jitter...),
                                    log.MDVP.Jitter.Abs. = log(patient.data$MDVP.Jitter.Abs.),
                                    log.MDVP.RAP = log(patient.data$MDVP.RAP),
                                    log.MDVP.PPQ = log(patient.data$MDVP.PPQ),
                                    log.MDVP.Shimmer = log(patient.data$MDVP.Shimmer),
                                    log.MDVP.Shimmer.dB. = log(patient.data$MDVP.Shimmer.dB.),
                                    log.Shimmer.APQ3 = log(patient.data$Shimmer.APQ3),
                                    log.Shimmer.APQ5 = log(patient.data$Shimmer.APQ5),
                                    log.MDVP.APQ = log(patient.data$MDVP.APQ),
                                    log.Shimmer.DDA = log(patient.data$Shimmer.DDA),
                                    log.NHR = log(patient.data$NHR),
                                    status = patient.data$status)

write.csv(logTransformed.patient.data, file = "weka.patient.data")
```
...
#TODO welke attributen vallen op
...
First of all how different are the sick values of each attribute against their healthy values? Each attribute will be tested with a Welch two-sample t-test. The data is copied to an csv file for clearer representation of the data.
...
#Welch.t.test(patient.data), "p.values")
pValues <- read.csv("p.values", header = TRUE, sep = ",")
#data.Pvalues <- data.frame(pValues, is_significant = c(apply(pValues, pValues$p.values < 0.05)))
#pander:pander(data.pValues)
```
