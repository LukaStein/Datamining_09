...
pander::pander(head(patient.data))
```
...
pander::pander(attributes, style = "rmarkdown")
```
...
```{r}
pander::pander(head(summary(patient.data)), style = "rmarkdown")
```
...
ggplot(difference.in.nums.healthy,aes(x=labs, y=values)) + geom_boxplot(fill = "aquamarine") + ggtitle("MDVP.Hz attributes of people without PD")
...
ggplots <- function(data, left, right) {
  ...
  numeric.attributes <- attributes[sapply(attributes, is.numeric)]
  cat(names(numeric.attributes))
  # pass columns to list via lapply
  hist.plots <- lapply(numeric.attributes, function(attribute_name) {
    ggplot(data, aes(x = attribute_name, fill = status)) + geom_histogram(color = "black")
  })
  return(hist.plots)
}

plots <- ggplots(patient.data, 1, 8)
plots
#plots[[1:8]]
#grid.arrange(plots, ncol = 1, nrow = 8)
```
...
ggplots <- function(data, left, right) {
  ...
  for (i in left:right) {
    if (is.numeric(data[, i])) {
      plot <- ggplot(data, aes(x=data[, i])) + geom_histogram(color = "black")
...
plots <- ggplots(patient.data, 1, 8)
grid.arrange(grobs = plots[2:4], ncol = 2, nrow = 4)

#ggplot(patient.data, aes(x=patient.data[, 2])) + geom_histogram(color = "black")
```
...
Though the pairplot shows similarity between groups healthy and sick. When a high correlation value is met between the pair it means, the data is very similar. An assumption is that high similarity shows there's no difference of that attribute for both groups which means that attribute may not be used to diagnose a PD patient.
