...
```
                                                        Shar                            ed histogram plot of last six attributes
...
Adding log-transformed data next to the original data set and writing the new dataframe to a csv file.


```{r}
combined.patient.data <- data.frame(patient.data, 
                                    log.MDVP.Fhi.Hz. = log(patient.data$MDVP.Fhi.Hz.),
...
#write.csv(combined.patient.data, file = "weka.patient.data")
```
...
```{r}
ggpairs(data.numeric[MDVP.small-2], ggplot2:: aes(color=patient.data$status), progress = FALSE, upper = "blank") + labs(caption = "Pair plot showing correlations between four MDVP attributes\n and showing how similar the status data is between them") + theme(axis.text.x = element_text(angle=90, hjust = 1),
        axis.text.y = element_blank())                                                                                                                                                                
```

The columns MDVP.PPQ and MDVP.RAP correlate the best. Though MDVP.RAP and MDVP.Jitter correlate also largely. More interesting MDVP.Jitter.Abs and MDVP.Flo.Hz show a large non-correlation, between sick and healthy. This combination might distinguish a person with PD from a healthy person.

```{r}
ggpairs(data.numeric[MDVP.shimmer-2], ggplot2:: aes(color=patient.data$status), progress = FALSE, upper = "blank") + labs(caption = "Pair plot showing correlations between four MDVP.shimmer attributes\n and showing how similar the status data is between them", legend = 1) + theme(axis.text.x = element_text(angle=90, hjust = 1), axis.text.y = element_blank(), legend.position = "bottom")                                                                  
```
