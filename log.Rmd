...
The last Explored mode will be J48 pruned tree
...
For the first test the algorithms Ibk, J48, ZeroR, OneR and NaiveBayes are tested with the default parameters

Dataset                   (1) trees.J4 | (2) bayes (3) lazy. (4) rules (5) rules
--------------------------------------------------------------------------------
patient_data_labels      (100)   84.79 |   70.14 *   95.91 v   75.39 *   87.70  
--------------------------------------------------------------------------------
                               (v/ /*) |   (0/0/1)   (1/0/0)   (0/0/1)   (0/1/0)
                               
                               
NaiveBayes is the least accurate and IBk is the most.


What would happen if for J48 the unpruning is set to true with a minNumObj to three? Also for OneR the bucketsize is set to 20.

Dataset                   (1) trees.J48 '-U  | (2) bayes.Naive (3) lazy.IBk '- (4) rules.ZeroR (5) rules.OneR 
--------------------------------------------------------------------------------------------------------------
patient_data_labels      (100)   84.83(7.85) |   70.14(9.30) *   95.91(4.52) v   75.39(1.87) *   85.10(5.54)  
--------------------------------------------------------------------------------------------------------------
                                     (v/ /*) |         (0/0/1)         (1/0/0)         (0/0/1)         (0/1/0)


J48 seems to increase in accuracy with a small amount, which means that either an increasing group of objects results in a better outcome or that a part is pruned that actually later become more accurate. OneR's accuracy decreases by about two after increasing the bucket size of six to twenty. Since the dataset has approximately 200 rows, ten buckets are made where a best attribute is chosen from. The less buckets are made in a dataset the chance then increases for errors to be made.


Next lets include the log dataset and see what dataset is handled better. An extra dataset can be added again under "Add new...".

Dataset                   (1) rules.Ze | (2) rules (3) trees (4) lazy. (5) bayes
--------------------------------------------------------------------------------
patient_data_labels      (100)   75.39 |   87.70 v   84.79 v   95.91 v   70.14  
log_patient_data         (100)   75.39 |   87.81 v   84.63 v   93.03 v   72.75  
--------------------------------------------------------------------------------
                               (v/ /*) |   (2/0/0)   (2/0/0)   (2/0/0)   (0/2/0)
                               
                               
For the log patient data J48 and IBk are a bit better This might be because the data is more similar thus less branches and classification lines are made.
Though what dataset is significantly better? Be applying Swap a * behind the algorithm should show if that's the case.

Dataset                   (1) patient_ | (2) log_p
--------------------------------------------------
rules.ZeroR '' 4805554146(100)   75.39 |   75.39  
rules.OneR '-B 6' -345942(100)   87.70 |   87.81  
trees.J48 '-C 0.25 -M 2' (100)   84.79 |   84.63  
lazy.IBk '-K 1 -W 0 -A \"(100)   95.91 |   93.03 *
bayes.NaiveBayes '' 59952(100)   70.14 |   72.75  
--------------------------------------------------
                               (v/ /*) |   (0/4/1)
                               
                               
It's clear that only IBk does significantly better on the log_p data. All other algorithms don't do any better significantly. This means that the log data is better to use for differentiating the levels.

The run time data can also be saved to csv by choosing CSV file under "Results destination", giving the desired location and then run the setup again.
...
There are quite a lot of interesting columns in this file. It shows that 175 of the instances are used for training and twenty for training. Then it's show how many number of times the levels are guessed correctly and incorrectly. The means of that data is shown in the tables above. Additionally to those tables a histogram is made of the amount of true positives, false positives, false negatives and true negatives.
