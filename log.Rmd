...
All models will be compared wtih different comparison fields. Let's also sort the results by pressing Sorting (asc.) by e.g. percent_correct. For each next run the "sort by" will be applied with its corresponding comparison field.
...
Let's count the occurences by eye of all top six attributes and make a barplot to visualize the best attributes all round. The countings are added in a file called "best_features_counts.txt".
...
In the barplot it becomes clear that five attirubtes: MDVP.APQ, MDVP.Flo.Hz, MDVP.Fo.Hz, MDVP.Jitter.Abs and spread1 occurred an equal amount of times in the top six.
...
As you can see the accuracy has increased by one percent. After testing other options it seems this will be the optimized model to use for predicting on the data.
...
\bibitem{F-measure defintion}
Zhang, E., Zhang, Y. (2009). F-Measure: \textit{F-Measure} Link : 
